...
const ACTIVIDADES = [
  { value: "empleado_formal", label: "Empleado formal", icono: "badge", hint: "Con CFDI de nómina" },
  { value: "empleado_informal", label: "Empleado informal", icono: "handshake", hint: "Sin contrato" },
  { value: "negocio_propio", label: "Negocio propio", icono: "store", hint: "Dueño de negocio" },
  { value: "independiente", label: "Freelance", icono: "laptop_mac", hint: "Honorarios" },
...
export default function Paso3SituacionEconomica({ onNext, onBack }: Props) {
  ...
  const [ingresoDisplay, setIngresoDisplay] = useState<string>(
    datos.ingresoMensual
      ? datos.ingresoMensual.toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : ""
  )

  const [pagoDeudaDisplay, setPagoDeudaDisplay] = useState<string>(
    datos.pagoMensualDeudas
      ? datos.pagoMensualDeudas.toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      : ""
  )
  ...
  useEffect(() => {
    const subscription = watch((value) => {
      ...
      debounceRef.current = setTimeout(() => {
        useSolicitudStore.getState().guardarPaso(3, value as Partial<Paso3Data>);
      }, 300);
    });
    return () => {
    ...
    <form onSubmit={handleSubmit(onNext)} noValidate>
        ...
        <div>
          ...
          >
            ...
            <Controller
              ...
              name="antiguedad"
              render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange} onOpenChange={setAntiguedadOpen}>
                  <SelectTrigger data-size="" className={cn("!h-[52px] w-full rounded-xl border-0 bg-transparent pl-4 pr-3 text-sm shadow-none focus:ring-0", antiguedadActual ? "pb-2 pt-6" : "py-0")}>
        ...
        <FloatingInput
          ...
          onChange={(e) => {
            ...
            if (!isNaN(num)) {
              ...
              setValue("ingresoMensual", undefined as unknown as number, { shouldValidate: true })
            }
          }}
          onBlur={() => {
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-3">
          {[
            { value: "si", label: "Sí, tengo deudas", icono: "credit_card_off" },
          ...
          <div>
            ...
            >
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange} onOpenChange={setMontoTotalOpen}>
                    <SelectTrigger data-size="" className={cn("!h-[52px] w-full rounded-xl border-0 bg-transparent pl-4 pr-3 text-sm shadow-none focus:ring-0", montoTotalDeudasActual ? "pb-2 pt-6" : "py-0")}>
                      <SelectValue placeholder="" />
                    </SelectTrigger>
                    <SelectContent>
          ...
          <FloatingInput
            ...
            onChange={(e) => {
              ...
              if (!isNaN(num)) {
                ...
                setValue("pagoMensualDeudas", undefined as unknown as number, { shouldValidate: true })
              }
            }}
            onBlur={() => {
